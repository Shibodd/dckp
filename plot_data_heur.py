...
algos = ["greedy", "hillclimb", "relax_ldckp"]
dfs = {
...
for corr, scale in pairs:
    ...
    # Format numbers
    sub_df['avg_solver_time'] = sub_df['avg_solver_time'].apply(lambda col: (col * 1e6).round().astype(int))
    ...
    sub_df = sub_df.rename(columns={
        "avg_solver_time": "Time [us]",
        "avg_optimality_gap": "Opt Gap [\%]"
    })
    
    latex_output = sub_df.to_latex(
        column_format="l" + "rr" * len(pivot_df.columns.levels[1]),
        caption=f"{corr}{scale} - inexact solvers",
        label=f"tab:{corr}{scale}inexact",
        escape=False,
        multicolumn_format='c',
        float_format="%.2f"
    )
    
    latex_output = latex_output.replace("relax_ldckp", "relax")
    print(latex_output)